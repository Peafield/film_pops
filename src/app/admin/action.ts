...
type ArchiveMoviesActionResult = {
	success: boolean;
	message: string;
...
const ARCHIVE_THRESHOLD_MONTH = 1;
...
export async function archiveOldMoviesAction(): Promise<ArchiveMoviesActionResult> {
	...
	try {
		...
		thresholdDate.setMonth(thresholdDate.getMonth() - ARCHIVE_THRESHOLD_MONTH);
		const thresholdDateString = thresholdDate.toISOString().split("T")[0];

		console.log(
			`[Archive Action] User ${session.user.id} (role: ${userRole}) is archiving movies released before: ${thresholdDateString}`,
		);
